...
note: "They consist of device drivers and system applications that optimize the guest operating system for better performance and usability." from [docs](https://docs.oracle.com/en/virtualization/virtualbox/7.1/user/guestadditions.html#guestadd-intro)
...
credit: final commands from [here](https://forums.virtualbox.org/viewtopic.php?t=63225)
...
3. on macos (unknown status on linux), virtualbox will not have access to the raw disk, even through this handle. add yourself to the operator group, which
does have those rights `sudo dseditgroup -o edit -a $USER -t user operator`
4. in virtualbox, add a hard disk to the usb controller (did not work with the other controller) via disk image file and select the .vmdk file
5. done
